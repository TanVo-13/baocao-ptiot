...
void callback(char *topic, byte *message, unsigned int length)
{
  ...
  if (String(topic) == "HeThongNhaThongMinh/DHT22/Control/automodeDht")
  {
    ...
    {
      automodeDht = true;
      Serial.println("Chuc nang tu dong da duoc bat");
    }
    else if (stMessage == "OFF")
    {
      automodeDht = false;
      Serial.println("Chuc nang tu dong da duoc tat");
    }
  }
}
...
void loop()
{
  ...
  if (millis() - lastSend > 2000)
  {
    ...
    // Kiểm tra nhiệt độ và điều khiển điều hòa
    if (!isnan(nhietdo) && automodeDht)
    {
      if (nhietdo > 30)
      {
        digitalWrite(dieuhoa, HIGH);
        Serial.println("Dieu hoa dang bat");
      }
      else
      {
        digitalWrite(dieuhoa, LOW);
        Serial.println("Dieu hoa dang tat");
      }
    }
